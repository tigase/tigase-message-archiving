...
public class MessageArchivePluginTest extends ProcessorTestCase {
	...
	public void testXEP0313_IngoringArchivizationMessagesRetrievedFromRepository() throws Exception {
		...
		Packet packet = Packet.packetInstance(new Element("message", new Element[]{
				new Element("result", new Element[] {
					    	new Element("forwarded", new Element[] {
					    		new Element("message", new Element[]{
									    new Element("body", "Test body messages")
							    }, new String[] { "from", "to" }, new String[] { "user2@example.com", "user1@example.com" } )
						    },new String[] { "xmlns" }, new String[] { "urn:xmpp:forward:0" })
						}, new String[] { "xmlns", "queryid", "id" }, new String[] { "urn:xmpp:mam:1", "g28", "28428-20978-43925" })
		}, new String[] { "from", "to" }, new String[] { "from@example.com/res1", "to@example.com/res2" }));
